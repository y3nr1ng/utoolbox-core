...
import utoolbox.container.dataset.mm.error as mm_error
import utoolbox.container.dataset.spim.error as spim_error
...
def analyze(ctx, root):
    def _try_dataset_flavors(root):
        try:
            return dataset.SPIMDataset(root, refactor=False)
        except spim_error.SettingsNotFoundError:
            logger.info("not generated by SPIM")

        try:
            return dataset.MicroManagerDataset(root)
